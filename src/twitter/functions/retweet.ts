...
export async function retweet(tweetId: string): Promise<RetweetResult> {
  try {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown retweet error';
      const message = `Failed to retweet: ${errorMessage}`;
      Logger.log(message);
      return {
        success: false,
        message
    ...
    const message = `Error retweeting tweet: ${errorMessage}`;
    Logger.log(message);
    return {
      success: false,
      message
